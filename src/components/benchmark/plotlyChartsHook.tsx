...
index_type: %{customdata[6]}<br>
index_create_parameter: %{customdata[7]}<br>
index_search_params: %{customdata[8]}<br>
<extra></extra>`;
...
function filterDataPoints(eachPoints: each_json[], pointProperty: keyof each_json): each_json[] {
  ...
  // 从当前元素的下一个元素开始搜索
    ...
    } else if (pointProperty === 'mean_time' || pointProperty === 'p95_time') {
      nextIndex = self.findIndex((v, i) => i > index && v[pointProperty] < value[pointProperty]);
...
export function refreshScatterChart(filtered_data: each_json[], plotlyLineChart: any, plotlyScatterChart: any, indicator: keyof each_json) {
  ...
  for (const group in groups) {
    ...
    for (const sub_group_parallel in sub_groups_by_parallel) {
      ...
      const trace_line = {
        ...
        customdata: each_data_line.map(d => [d.engine, d.version, d.remark, d.dataset, d.dataset_tag, new Date(d.time_stamp), d.index_type, JSON.stringify(d.index_create_parameter), JSON.stringify(d.index_search_params)]),
      ...
      const trace_scatter = {
        ...
        customdata: each_data_scatter.map(d => [d.engine, d.version, d.remark, d.dataset, d.dataset_tag, new Date(d.time_stamp), d.index_type, JSON.stringify(d.index_create_parameter), JSON.stringify(d.index_search_params)]),
        hovertemplate: hovertemplate,
        marker: { size: 8 },
      };
  ...
  let layout = {
    yaxis: {
      ...
      title: indicator==='rps'?'RPS (query/s)':'Time (s)', // y 轴的单位是米
      showline: true,
    },
  };
  // 处理生成 xaxis layout
  ...
  for (let i = 0; i < generated_xaxis_list.length; i++) {
    ...
    if (i != generated_xaxis_list.length - 1) {
      layout_xaxis[key] = { domain: [(gap + blank) * i, (gap + blank) * i + gap], title: "mean_precision"  };
    } else {
      layout_xaxis[key] = { domain: [(gap + blank) * i, 1], title: "mean_precision" };
    }
    dynamic_annotations.push({
      ...
      xanchor: 'center',
      yanchor: 'bottom',
      text: 'Search parallel: '+generated_xaxis_list[i],
      showarrow: false,
      font: {
        size: 12
      }
